...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    if [ -z "$MONGODB_URI" ]; then
        echo "❌ MONGODB_URI is not set. This app uses MongoDB (not Prisma/SQLite)."
        echo "   Set MONGODB_URI in your deployment environment (e.g. AWS App Runner)."
        exit 1
    fi

    echo "🗄️  Database: MongoDB Atlas (MONGODB_URI)"
